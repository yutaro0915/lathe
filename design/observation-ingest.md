...
- **Codex sub-agent**: 親 rollout の `spawn_agent` function call と同じ `call_id` の
  `function_call_output` に `{"agent_id":"..."}` が返る。子は独立 rollout session として保存され、
  子の `session_meta.source.subagent.thread_spawn.parent_thread_id` に親 session id が入る。Lathe は
  親の launcher event に `agent_id` を保持し、同じ id の child session が DB に存在する場合だけ
  `sessions.parent_session_id` / `sessions.spawned_by_seq` と launcher `meta.child_session_id` を張る。
...
### Claude Code sub-agent の扱い

Claude Code の `<session>/subagents/agent-*.jsonl` は Lathe では独立 session にせず、親 transcript ingest 時に
Agent/Task launcher の `parent_id` 配下の child `transcript_events` として展開している。したがって
Claude 側には `sessions.parent_session_id` を張る対象の child `sessions` row が無い。これを session 化すると
既存の親 session 内 child event 表示と session-level usage 集計が二重化するため、#19 では Codex 独立 rollout
のみを session parent link の対象にする。
