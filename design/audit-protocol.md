...
7. **全実装に独立レビュー必須（2026-06-12 ユーザー決定）**: tier に関わらず、すべての実装は
   ...
   **レビューに使うモデル（2026-06-12 ユーザー訂正・厳守）**: **最新 Opus**（`claude-opus-4-8`、
   ...
   最終防壁であり、ここをケチらない。
   **Tier A の監査（独立レビュー）は Codex（xhigh+）で行う（2026-06-13 ユーザー指示）**。実装は Opus の
   ことが多く、別モデル系統の Codex の方が独立性が高い（+ 別枠）。Tier B/C の独立レビューは Opus / Codex の
   どちらでもよい（Sonnet 禁止は全 tier 共通）。main（Fable）はレビューを自分でやらず委譲する（監督に徹する）。
