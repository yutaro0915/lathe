...
7. **全実装に独立レビュー必須（2026-06-12 ユーザー決定）**: tier に関わらず、すべての実装は
   merge 前に **sub-agent（コードレビュー観点で別 context）または Codex** による独立レビューを
   必ず通す。実装者と同じ context（= main 自身の diff 流し読みのみ）で済ませない。レビュー指摘は
   原則どおり実コードで裏取りしてから採否（誤検出を block にしない）。chip 由来 PR も同じく、
   merge 前に独立レビューを 1 回かける。
