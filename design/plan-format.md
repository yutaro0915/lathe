...
| クラス | 例 | 要求 |
|---|---|---|
| **trivial** | 明確なバグ修正・数行・契約/構造に触れない | **軽量形**: 問題 / 修正方針 / 検証 / 見積り の4行〜。承認不要（既存の「低リスク小変更は軽量 plan で可」を維持） |
| **standard** | 機能追加・複数ファイル・契約/構造に触れる | **完全形**（下記6セクション）＋ needs-approval なら PdM 承認 |
...
6. **見積り** — diff 規模（変更ファイル数・行数の見通し）と想定 implement 分数（目安）。正準フォーマット（1 行）: `見積り: diff ~<N> 行 / <M> ファイル・implement ~<T> 分`。実測が大幅に超過した場合は meta finding として観測される（ADR 0030 §5 粒度規準。分割強制はしないが、観測点として使う）
...
- **見積り無宣言・scope に対し過小な見積りは PLAN_REVIEW が差し戻す**
