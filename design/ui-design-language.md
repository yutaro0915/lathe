...
## コピー原則（2026-06-12 ユーザー指摘で追加）

1. **UI の文言は中立・無感情の英語 micro-label に統一**（既存の COST BY MODEL / WHERE THE
   ACTIONS WENT と同調）。砕けた日本語・編集的な言い回し（「次に掘る場所」「〜しようぜ」等）・
   日英混在文は**禁止**。指示文・設計文の言い回しを製品コピーに持ち込まない。
2. **判定的な表示には必ず根拠を添える**。例: cost 異常は「COST ALERTS」と断定せず、
   `COST OUTLIERS — >5× runner median (G9)` のように基準を明示。基準のない数字を
   見出しに昇格させない。
3. 空状態は `none` / `no data` 等の最小表現。煽り・励まし・お世辞コピーを書かない。

## IA 原則（2026-06-12 ユーザー決定）

1. **全画面は常設グローバルバー配下**。バー・タブ以外の手段でしか到達/離脱できない画面を
   作らない（「今どこか」「どう戻るか」が常に自明）。見た目の異なる別画面への遷移は禁止。
2. **横断軸と session 軸を混ぜない**。横断テーマ（Findings 等）はグローバルバーの一級軸。
   session viewer 内のタブは**当該 session に紐づく内容のみ**を表示する（横断一覧を
   session タブに同居させない）。
3. グローバルバー構成: Sessions / Findings / PR / Overview。**Chat は当面ナビに置かない**
   （構想 = 画面でなく全画面に被さるレイヤー。ROADMAP 論点 #16、意図的保留）。
