...
- 正本 ADR: [adr/0011-rubric-schema-v2.md](../adr/0011-rubric-schema-v2.md)
- date: 2026-06-23

ADR 0011 の決定を実装可能な仕様に落とす。EDD 前線1 = **正しさの SSOT（rubric）の形式標準化**。判定挙動は不変（記録のみ）。
...
```jsonc
{
  ...
  "version": "1",
  "scope": ["rubrics"],
  "origin": "EDD 前線1 / ADR 0011 (2026-06-23)",
  "checks": [
    {
...
- `run.mjs` は touch しない（判定挙動不変が ADR 0011 の核）。
