...
- 永続層: `findings` / `finding_evidence` / `finding_verdicts` / `harness_versions` /
  `chat_threads` / `chat_messages`（§6.4）
- ~~annotations も永続層へ~~ → **訂正（2026-06-11 監査）**: annotations は provider が transcript
  から自動生成する**導出データ**であり導出層に留める（永続層化すると再 ingest との二重管理・
  孤児化が起きることを実 DB で確認）
- **版数の同一性規約（2026-06-11 監査で追加）**: harness 版 hash の対象は **git 管理下の
  harness artifact のみ**（untracked / `*.local.*` はマシン固有のため除外）。live（hook）採取と
  git 再構成は**同一の収集規則を共有**し、同一 repo 状態なら同一 hash になること自体を検証する。
  notify でスタンプ済みの版を sweep が上書きしない（既存スタンプ優先）。
  branch tip からの再構成はしない（commit を特定できる場合のみ。できなければ NULL — 捏造しない）
- **検証の分離**: verify スクリプトは共有 DB に test データを残さない（scratch DB / 分離 schema で
  実行し、後始末まで含めて検証）。resetDatabase を verify が直接叩かない
