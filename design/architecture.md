...
> **強制**: 本書の **構造不変条件（§5）** は oxlint + dependency-cruiser が機械強制。**コードの振る舞い規範**は [engineering-norms.md](engineering-norms.md) の N1–N8（別軸・補完）。
...
- 役割分担を明確にする:
  - **engineering-norms.md（N1–N8）** = コードの振る舞い規範（反証可能ゲート・provenance 保持・scratch 隔離 等）。
...
- **Finding 集約の不変条件**: verdict は二値（accept/reject、本体に埋めず別テーブル）。backlog は `open → addressed | dismissed` のみ遷移。analysis は上位（agent 生成）を下位 backfill で**上書きしない**（[engineering-norms.md](engineering-norms.md) N3）。
...
- [engineering-norms.md](engineering-norms.md)（N1–N8）= 振る舞い規範。本書 = 構造。両者は補完。
...
- [stacking-norms.md](stacking-norms.md) / [workflows.md](workflows.md) = 収束を PR スタックで運ぶ運用。
