...
| **解説（explain）** | explain-diff skill を読める任意のセッション（runner 自動化は任意拡張、ADR 0032/0033） | `explain` label の issue 到着・PdM の直接要求 | 参照対象（PR／plan／ADR／概念）に接地した GFM 教材の生成＋禁則の機械検収 | **正本保存（`explains/`）＋ Discussion 投稿（issue 起点ならリンク comment＋close）** | 対象への裁定・実装・レビュー・Quiz 採点等のテレメトリ |
