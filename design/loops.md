...
|---|---|---|---|---|---|
| **inner（task）** | driver `scripts/inner-loop.mjs` + named agents | backlog task | PLAN→IMPLEMENT→REVIEW→VERIFY→(TRIAGE)→MERGE を自律完走 | **ゲート経由の merge**、または **escalation（停止して outer の判断待ち）** | outer による代行完走・ゲート迂回 |
