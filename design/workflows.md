...
**拡張（2026-06-12 ユーザー指示）**: 実装に限らず、**作業内容が明確に仕様化できるタスク
（範囲確定の調査・診断・一括変換・文言整理等）は Opus / Sonnet サブエージェントへ委譲**する。
main（Fable）は「ユーザー対話を伴う全体調整 / タスク割り振り / コードレビュー・リファクタ判断 /
高度な関係付け・意図の抽出 / 仕様化困難な複雑作業」に集中する。
配分（2026-06-13 ユーザー更新、Claude レート温存で Codex 優先）: **UI 実装 = Opus / その他の実装 = Codex `gpt-5.5` high / 重要レビュー（Tier A 監査含む）= Codex `gpt-5.5` xhigh**。
UI 以外に Opus を使わない。軽い実装は Codex を基本としつつ **Codex のレート次第で Sonnet を使うのは可（柔軟に、2026-06-13）**。ただしレビューは Sonnet 禁止。判断基準は「prompt に書き切れるか」— 書き切れるなら main がやらない。
...
# 2026-06-14 ユーザー指示: -a never で起動（承認プロンプト無し → 承認 watcher 廃止）。
# network/docker/git が要るので -s danger-full-access。effort は high（重要レビューは別途 xhigh）。
tmux new-session -d -s lathe-loop-<NN> -c <worktree> \
  "codex --no-alt-screen -a never -s danger-full-access -c model_reasoning_effort=high '/goal <goal文>'"
...
- **承認の運用（2026-06-14 改定、ユーザー指示「`-a never` にするように」）**: loop は `-a never -s danger-full-access` で起動し、
  **承認プロンプトを出さない**。これにより「bash ポーリングで承認を肩代わりする watcher」を**廃止**する
  （旧 watcher は escalation / 停止検知 / 時間上限のたびに自分で exit する設計で churn が多く、環境による kill もあった）。
  - **安全側の担保**: loop は必ず **独立 detached/専用 worktree + scratch DB** で走らせ、goal に「push / main への merge / 共有 lathe DB の破壊（DROP/DELETE/TRUNCATE）/ force push をしない」を明記する。**git push と main への merge は Claude が手動**で握る（loop の成果は branch に置くだけ）。per-command の生ゲートは無くなるが、隔離 + 手動 merge + 事後 diff レビューで担保する。
  - **監視**: 承認は不要なので、**完了検知だけの軽い waiter**（`tmux capture-pane` で "Worked for" / session 消失を polling、エスカレーションしない）に置き換える。churn は出ない。
  - 旧方式（既定の対話承認 + prefix 永続許可 + 承認 watcher）は廃止。
