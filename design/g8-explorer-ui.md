...
1. 探索モデルの採用範囲: **A-1 のみ / A-1→A-2 / A-3 まで**（§3）
2. turn rollup の項目セット（§4 の表から取捨）
3. ファイル軸（§5）を A-1 と同時に入れるか、後続 task に分けるか
4. 実装 task の分割単位と受け入れ条件化（E2E で検証可能: 「session を開くと turn ヘッダのみ表示」
  「turn 行に rollup が出る」等は機械検証できる）
5. SessionViewer.tsx の分割（棚卸し所見: 単一コンポーネント 1400 行超。状態管理の層分離を
  この再設計と同時にやるか）

## 8. 出口

レビュー → 未決 1〜5 の決定 → tasks/09 以降に受け入れ条件付きで分割 → goal loop（dev-loop v2）で消化。
本ノートは決定後、決定内容を ADR 化（または ROADMAP の Phase 1.5 として追記）して参照頻度を下げる。
