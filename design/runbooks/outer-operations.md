...
- 裁定は必ず **task（issue）に記録**する（issue comment または PR コメント。ADR 0031。
  監査痕跡。判断を消さない）。
...
新規 task の発行は **GitHub Issue に `task-request` label を付ける**だけ（ADR 0031）。
**issue がそのまま task**（TASK-N = issue #N。採番は GitHub が直列化。写し Action は廃止）。
却下は無い——priority（label `p0-urgent`/`p1-high`/`p3-low`、無指定は medium）や採否の
triage は PdM が GitHub Projects 盤面で行う（Projects はビュー。機械は labels と issue
状態のみ読む）。status は保存せず導出: open=To Do／参照 PR open=In Progress／merge close=Done。
plan は issue body、裁定・申し送りは issue comment、依存は body の `blocked-by #N`。
