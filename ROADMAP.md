...
| 17 | UX backlog（2026-06-12 ユーザーレビュー、今回は未着手と明言された分）: PR 軸は project 単位のグルーピングで見る目線が欠落 / 上部（ヘッダ・タブ帯）のスペース使いが下手 / 左 session rail がフィルタで埋まり判読不能 / Annotations タブの存在意義 / error の意味論（何をエラーと数えているか・どこで起きたか追えない） | 残 | 次回 UX ラウンドで設計から。error 意味論は診断結果（subagents/errors 調査）を踏まえて task 化 |
