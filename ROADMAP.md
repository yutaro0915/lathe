...
| 16 | chat の設計（どこからでも呼び出せる / 過去会話をどこでも参照 / 今見ている画面の要素を選択してコンテキスト化 = 画面でなく全画面に被さるレイヤー） | **コード撤去（2026-06-12 決定）** | ユーザー決定: 休眠コードを残さず**今撤去**（/chat・ChatView・chat-agent/store/tools・chat API・chat e2e）。構想は design メモに残し**再開時はゼロから実装**。issue #7 は撤去で自動解消（close）。tasks/18 の実装は git 履歴に保存 |
