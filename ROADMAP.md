...
| 16 | chat / agent 機能 = **Phase 2.5 として実装（2026-06-13 ユーザー決定）** | **P2.5 にスケジュール** | 2026-06-12 にコード撤去（ゼロから再実装方針）→ 2026-06-13「P2.5 で chat/agent を実装する」と near-term 化。構想は全画面に被さるレイヤー（どこからでも呼べる / 過去会話参照 / 画面要素を選択してコンテキスト化）。**findings/backlog/depth の設計は P2.5 chat/agent との接続余地を残す**（finding を agent と議論 / agent に改修案を起こさせる 等）。現 findings はあくまで「仮」の体で進める |
