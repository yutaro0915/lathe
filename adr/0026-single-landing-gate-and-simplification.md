...
- **review の主張 → PR review が置換**（verdict と本文を PR に投稿。内容つき・sha 紐付き・
  ...
  **【修正 2026-07-05・ADR 0028】** required review は不採用——単一アカウント運用では
  author の self-approval が GitHub 仕様で不可能なため全 PR が deadlock する。review の担保は
  「verdict＋本文の PR 投稿（non-blocking）＋ CI の gate 再実行」。着地は auto-merge 標準・
  governance path のみ label tripwire。正本: [ADR 0028](./0028-unattended-landing.md)
