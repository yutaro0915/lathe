...
- date: 2026-06-23
- 関連: EDD v1 作業理論（cherie の作業理論。要点: 正しさ=eval/rubric と 手続きの正しさ=harness/run-validity を直交2軸に分離）/ ROADMAP「計画運用 — rolling wave と監査」/ [0005](./0005-harness-artifact-model.md)（harness artifact model）/ `rubrics/run.mjs`
...
- EDD v1 作業理論: cherie の作業理論（未完成・生きたドキュメント。要点: 正しさ=eval/rubric と 手続きの正しさ=harness/run-validity を直交させ、rubric を「成果物を拒絶する独立した理由」の単位で持つ。詳細は別管理）
