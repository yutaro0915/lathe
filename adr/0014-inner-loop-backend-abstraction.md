...
| 権限 | `--permission-mode`＋`--allowedTools` | `-s read-only`（PLAN/REVIEW/VERIFY/TRIAGE）／`-s workspace-write`（IMPLEMENT、`-C`=worktree）。`--dangerously-bypass-*` は使用禁止 |
...
### 2. 配分の既定と override
- **既定 = inner の全段 codex**（ユーザー意図: Claude token を fable/outer に温存）。
