...
test("postgres int8 parser preserves Lathe numeric row contracts", async () => {
...
test("getPool() returns same instance for same URL (singleton)", async () => {
...
test("getPool() returns new instance when DATABASE_URL changes", async () => {
  const saved = process.env.DATABASE_URL;
...
test("getPool() after closePool() returns a fresh instance", async () => {
  const saved = process.env.DATABASE_URL;
