...
test.describe("Cross-screen navigation & time ribbon", () => {
  test("Git is an in-page tab: diff shows in place, no navigation", async ({
    ...
    await page.locator(`[data-testid="tabs"] [class~="tab"]`, { hasText: "Git" }).click();
  ...
  test("from the Git tab, other tabs switch in-page (no /diff page)", async ({ page }) => {
    ...
    await expect(page).not.toHaveURL(/\/diff/);
    await page.locator(`[data-testid="tabs"] [class~="tab"]`, { hasText: "Transcript" }).click();
    await expect(page.locator(`[data-testid="tabs"] [class~="tab"][class~="active"]`)).toHaveText(/Transcript/);
    await expect(page.locator(`[data-testid="timeline"] [class~="event-row"]`).first()).toBeVisible();
  });
  ...
  test("time ribbon zoom widens the track", async ({ page }) => {
    ...
    await page.locator(`[data-testid="ribbon"] [class~="minimap-zoom"] button`, { hasText: "+" }).click();
...
test.describe("Event detail panel", () => {
  test("shows compact stats (duration/exit) and a wrapping output", async ({ page }) => {
    ...
    const bashRow = page
      ...
      .filter({ has: page.locator(`[data-testid="event-icon"][class~="bash"]`) })
      .first();
    if ((await bashRow.count()) > 0) {
      await bashRow.click();
      await expect(page.locator(`[data-testid="stat-strip"] [class~="stat"]`).first()).toBeVisible();
      await expect(page.locator(`[data-testid="code-block"][class~="output"]`)).toBeVisible();
      const ws = await page
        .locator(`[data-testid="code-block"][class~="output"]`)
        .evaluate((el) => getComputedStyle(el).whiteSpace);
      expect(ws).toBe("pre-wrap"); // output wraps, no horizontal cut-off
      // the old tall key/value table is gone
      await expect(page.locator(`[data-testid="detail"] [class~="kv"] dt`)).toHaveCount(0);
    }
  });
});
...
test.describe("Thinking", () => {
  test("thinking events are captured and viewable", async ({ page }) => {
    ...
    const trow = page
      .locator(`[data-testid="event-row"]`)
      .filter({ has: page.locator(`[data-testid="event-icon"][class~="thinking"]`) })
      .first();
    if ((await trow.count()) > 0) {
      await trow.click();
      await expect(page.locator(`[data-testid="detail-head"] [class~="dtitle"]`)).toHaveText(/Thinking/);
      const body = (await page.locator(`[data-testid="code-block"][class~="output"]`).innerText()).trim();
...
test.describe("Sub-agent expansion", () => {
  test("sub-agent rows expand to reveal child steps (tools/skills)", async ({ page }) => {
    ...
    const saExpander = page
      .locator(`[data-testid="event-row"]:not([class~="turn-header"])`)
      .filter({ has: page.locator(`[data-testid="event-icon"][class~="subagent"]`) })
      .first()
    ...
    if ((await saExpander.count()) > 0) {
      ...
      await expect
        .poll(async () => page.locator(`[data-testid="event-row"][class~="child-row"]`).count())
        .toBeGreaterThan(0);
      ...
      await expect(page.locator(`[data-testid="event-row"][class~="child-row"]`).first()).toBeVisible();
    }
  });
});
...
test.describe("Time ribbon & annotations", () => {
  ...
  test("ribbon: zooming in adds more time-axis ticks", async ({ page }) => {
    await gotoViewer(page);
    const before = await page.locator(`[data-testid="ribbon-axis"] [class~="tick"]`).count();
    await page.locator(`[data-testid="minimap-zoom"] button`, { hasText: "+" }).click();
    await page.locator(`[data-testid="minimap-zoom"] button`, { hasText: "+" }).click();
    await expect
      .poll(async () => page.locator(`[data-testid="ribbon-axis"] [class~="tick"]`).count())
  ...
  test("annotations are labelled (kind + step) and jump on click", async ({ page }) => {
    ...
    const ann = page.locator(`[data-testid="annotations-tab"] [class~="annotation"]`).first();
...
test.describe("Annotations tab (moved out of the right aside)", () => {
  ...
  test("there is an Annotations tab with a count badge, and the aside no longer hosts annotations", async ({
    ...
    const tab = page.locator(`[data-testid="tabs"] [class~="tab"]`, { hasText: "Annotations" });
    await expect(tab).toBeVisible();
    const count = await page.locator(`[data-testid="annotations-tab"] [class~="annotation"]`).count();
    if (count > 0) {
    ...
    await expect(page.locator(`[data-testid="aside"] [class~="annotations"]`)).toHaveCount(0);
  });

  test("annotations are listed in time order (at_seq ascending)", async ({ page }) => {
    await page.goto(`/?session=${SID}&tab=annotations`);
    const seqs = await page
      .locator(`[data-testid="annotations-tab"] [class~="annotation"]`)
      .evaluateAll((rows) =>
...
test.describe("Turn-first explorer", () => {
  ...
  test("initial transcript view shows turn headers only", async ({ page }) => {
    ...
    await expect(page.locator(`[data-testid="timeline"] [class~="event-row"][class~="turn-header"]`).first()).toBeVisible();
    await expect.poll(async () => page.locator(`[data-testid="timeline"] [class~="event-row"][class~="step-row"]`).count()).toBe(0);
  });

  test("turn headers show rollup values from the real session data", async ({ page }) => {
    const first = (await getTurnExpectations(SID))[0];
    await page.goto(`/?session=${SID}`);
    const row = page.locator(`[data-testid="timeline"] [class~="event-row"][class~="turn-header"][data-turn="${first.turn}"]`);
  ...
  test("turns with errors carry the error emphasis hook", async ({ page }) => {
    ...
    await page.goto(`/?session=${SID}`);
    const row = page.locator(`[data-testid="timeline"] [class~="event-row"][class~="turn-header"][data-turn="${errorTurn!.turn}"]`);
  ...
  test("turn row click expands and collapses; sub-agent nesting still expands", async ({ page }) => {
    ...
    const firstHeader = page.locator(`[data-testid="timeline"] [class~="event-row"][class~="turn-header"]`).first();
    await firstHeader.click();
    await expect.poll(async () => page.locator(`[data-testid="timeline"] [class~="event-row"][class~="step-row"]`).count()).toBeGreaterThan(0);
    await firstHeader.click();
    await expect.poll(async () => page.locator(`[data-testid="timeline"] [class~="event-row"][class~="step-row"]`).count()).toBe(0);

    await expandAllTurns(page);
    const saExpander = page
      .locator(`[data-testid="event-row"][class~="step-row"]:not([class~="turn-header"])`)
      .filter({ has: page.locator(`[data-testid="event-icon"][class~="subagent"]`) })
      .first()
      .locator(`[data-testid="tw-expand"]`);
    if ((await saExpander.count()) > 0) {
      await saExpander.click();
      await expect
        .poll(async () => page.locator(`[data-testid="timeline"] [class~="event-row"][class~="child-row"]`).count())
        .toBeGreaterThan(0);
    }
  });
  ...
  test("expanded step rows expose proportional time bars", async ({ page }) => {
    await page.goto(`/?session=${SID}`);
    await expandAllTurns(page);
    const bars = page.locator(`[data-testid="timeline"] [class~="event-row"][class~="step-row"] [class~="step-timebar"]`);
  ...
  test("turn files chip opens the active diff file; touched steps jump back", async ({ page }) => {
    await page.goto(`/?session=${SID}`);
    const chip = page.locator(`[data-testid="timeline"] [class~="turn-files-chip"]`).first();
    await expect(chip).toBeVisible();
    const fileId = await chip.getAttribute("data-file-id");
    expect(fileId).toBeTruthy();
    await chip.click();
    await expect(page.locator(`[data-testid="tabs"] [class~="tab"][class~="active"]`)).toHaveText(/Git/);
    await expect(page.locator(`[data-testid="file-row"][class~="active"][data-file-id="${fileId}"]`)).toBeVisible();
    ...
    await expect(page.locator(`[data-testid="tabs"] [class~="tab"][class~="active"]`)).toHaveText(/Transcript/);
    await expect(page.locator(`[data-testid="timeline"] [class~="event-row"][class~="selected"]`)).toHaveCount(1);
  });
  ...
  test("event type filters can highlight or hide non-matching steps", async ({ page }) => {
    ...
    await page.locator(`[data-testid="transcript-filters"] [class~="event-type-badge"][class~="bash"]`).click();
    await expect
      .poll(async () => page.locator(`[data-testid="timeline"] [class~="event-row"][class~="step-row"][class~="filter-dimmed"]`).count())
      .toBeGreaterThan(0);
    expect(await page.locator(`[data-testid="timeline"] [class~="event-row"][class~="step-row"] [class~="event-icon"][class~="bash"]`).count()).toBeGreaterThan(0);
    await page.locator(`[data-testid="filter-mode"] button`, { hasText: "Hide" }).click();
    await expect.poll(async () => page.locator(`[data-testid="timeline"] [class~="event-row"][class~="step-row"] [class~="event-icon"][class~="bash"]`).count()).toBe(0);
  });
});
...
test.describe("Transcript: turn grouping", () => {
  ...
  test("turn headers carry the Turn N · M steps chip", async ({ page }) => {
    const first = (await getTurnExpectations(SID))[0];
    await page.goto(`/?session=${SID}`);
    await expect(page.locator(`[data-testid="event-row"][class~="turn-header"]`).first()).toBeVisible();
    await expect(page.locator(`[data-testid="chip"][class~="turn-chip"]`).first()).toContainText(/Turn 1\b/);
    await expect(page.locator(`[data-testid="event-row"][class~="turn-header"]`).first()).toContainText(`${first.steps} step`);
  });

  test("Expand turns restores step rows; Collapse turns returns to turn headers only", async ({ page }) => {
    await page.goto(`/?session=${SID}`);
    const headers = await page.locator(`[data-testid="event-row"][class~="turn-header"]`).count();
