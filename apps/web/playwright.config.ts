...
export default defineConfig({
  ...
  webServer: {
    ...
    command: `pnpm build && sh -c './node_modules/.bin/next start -p ${PORT} & pid=$!; trap "kill $pid 2>/dev/null; wait $pid 2>/dev/null; exit 0" TERM INT; wait $pid'`,
    env: { LATHE_CHAT_ACP_ADAPTER: "fake" },
