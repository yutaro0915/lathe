...
pnpm ingest        # ~/.claude/projects/<project-dir>/*.jsonl を Postgres に取り込み（既定は最新の Claude プロジェクト）
...
pnpm e2e           # Playwright E2E（49 ケース。build+start を内部で回すので dev は止めてから）
```

注意:
- **DB は再生成物**。clone 後は Postgres を起動し、必ず `pnpm ingest`。
- **dev 稼働中に `pnpm build` / `pnpm e2e` を走らせない**（同じ `.next` を壊す）。E2E/ビルド前に dev を止める。
- `pnpm ingest` 後に dev が起動済みなら**再起動**する。接続先は `DATABASE_URL` で差し替え可能（dev 既定値は `.env.example`）。
