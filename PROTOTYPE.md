...
- env: `DATABASE_URL` / `LATHE_TRANSCRIPTS_DIR`（**既定は最も最近活動した Claude プロジェクト dir を自動選択**＝ハードコードなし、誰の環境でも動く）/ `LATHE_CODEX_PROJECT`（Codex を cwd basename で絞り込み、既定は上記 dir から導出）/ `LATHE_NO_CODEX=1`（Codex 無効化）。
