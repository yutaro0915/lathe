...
**現在の状態（2026-06-10）**: Phase 1（観測 = トランスクリプト / Git 差分 / 統計ビューア）プロトタイプ**Postgres 移行済み・E2E 49/49 GREEN・coverage GREEN**。
...
Next.js + Postgres の Web UI で観測する。提案書の白い実装イメージ `phase-1-*.png` を正本にしている。
...
pnpm -F web ingest        # catch-up sweep: 既存 JSONL を Postgres に取り込み（既定は最新の Claude プロジェクト）
export LATHE_NOTIFY_TOKEN="$(node -e "console.log(require('node:crypto').randomBytes(32).toString('base64url'))")"
pnpm -F web dev           # next dev（既定 http://localhost:3000、`.claude/launch.json` 経由なら :3210）

pnpm -F @lathe/client build
# 観測対象 repo 側で @lathe/client を install 済みなら:
lathe-client init --server-url http://localhost:3000
# 対象 repo で Claude Code / Codex の turn が終わると Stop hook → /api/ingest/notify で自動反映

pnpm -F web build         # 本番ビルド（型チェック込み）
pnpm -F web coverage      # 正本(JSONL) ⇄ DB の機械照合（GREEN を確認）
pnpm -F web e2e           # Playwright E2E（49 ケース。build+start を内部で回すので dev は止めてから）
```

注意:
- **DB は再生成物**。clone 後は Postgres を起動し、必ず `pnpm -F web ingest`。
- **dev 稼働中に `pnpm -F web build` / `pnpm -F web e2e` を走らせない**（同じ `.next` を壊す）。E2E/ビルド前に dev を止める。
- `pnpm -F web ingest` 後に dev が起動済みなら**再起動**する。push ingest は Stop hook ごとに単一 session を差し替えるので、catch-up sweep の手動再実行はサーバ停止中の取りこぼし回収用。接続先は `DATABASE_URL` で差し替え可能（dev 既定値は `.env.example`）。
- env: `DATABASE_URL` / `LATHE_NOTIFY_TOKEN`（`/api/ingest/notify` の Bearer token。server と client init で同じ値を使う）/ `LATHE_TRANSCRIPTS_DIR`（**既定は最も最近活動した Claude プロジェクト dir を自動選択**＝ハードコードなし、誰の環境でも動く）/ `LATHE_CODEX_PROJECT`（Codex を cwd basename で絞り込み、既定は上記 dir から導出）/ `LATHE_NO_CODEX=1`（Codex 無効化）。
...
`scripts/ingest.ts`（`pnpm -F web ingest`）が実 JSONL を catch-up sweep として取り込む:
...
`POST /api/ingest/notify` は Bearer token を必須にし、hook payload の `{agent, session_id, transcript_path, cwd, project_id, event}` だけを受ける。`transcript_path` は `~/.claude/projects` / `~/.codex/sessions` / `~/.codex/archived_sessions` 配下の `.jsonl` に制限し、該当 transcript 1 本を provider で再解析して当該 session の DB 行だけを冪等に差し替える。hook は本文を運ばず、サーバ停止中の取りこぼしは上記 catch-up sweep で回収する。
...
- **Postgres を使用**（dev は `docker-compose.dev.yml` の依存コンテナ、アプリは host 実行）。接続先は `DATABASE_URL`。
...
- **Cursor トランスクリプト対応**: provider 抽象の先出しリファクタは完了済み。`scripts/ingest.ts` は provider loop と DB 書き込み呼び出しに限定し、Claude / Codex は `scripts/ingest/providers/`、共通型は `scripts/ingest/built.ts` / `providers/types.ts`、共有表示 helper は `lib/format.ts` / `lib/event-display.ts` / `lib/runner-display.ts` に集約済み。次は Cursor の transcript 形式を調査し、`scripts/ingest/providers/cursor.ts` を追加する。
