...
2. **token / theme** — `apps/web/app/design-system/tokens.css`（色・spacing=**4px グリッド**・radius・明暗）。**UI の数値は全部ここから**。生 px / 生 hex は書かない
3. **primitive** — `apps/web/components/ds/`（Button / Badge / Chip / Select / Panel / TabBar / Surface / Step …）＝再利用部品の**唯一の正本**
...
1. **既存 primitive を探す**（`components/ds`）。あれば**それを使う**。
...
- **dep-cruiser**: feature 同士の deep-import 禁止 / primitive は `components/ds` からのみ
...
関連: [[memory/feedback_systemic_enforcement]] / `design/ui-design-language.md` / `design/layout-architecture.md` / `rubrics/apps/web/styling/`
