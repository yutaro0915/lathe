...
- **生成 agent（publish 前・必須）**: 禁則語 grep（昨日/門番/運命/勘所 等）・4 節の存在・
  ...
  **Quiz 正解位置の照合**: 各設問 k（1 始まり）の `<details>` 直前の正解先頭文字（a–d）が
  `(N+k) mod 4`（0=a/1=b/2=c/3=d）の規定位置と一致することを確認する。
- **監査役（publish 後・ブラウザ系ツールを持つ環境で）**: 実描画確認（mermaid iframe が
  ...
  あわせて **Quiz 正解位置の機械検査**: `grep -h -A3 "<summary>答えと解説</summary>" <file> | grep -oE "^\*\*[a-dA-D]"` で
  各設問の正解先頭文字を抽出し、設問番号 k（1 始まり）ごとに `(N+k) mod 4`（0=a/1=b/2=c/3=d）と照合する。
  不一致があれば不合格（再生成）。
