...
4. **Quiz**: 中難度の 5 問（実質を理解していないと解けないが、ひっかけでない）。
   ...
   **正解の選択肢位置は決定的配置規則に従う**（LLM の位置バイアスを排除・issue #258）:
   設問 k（1 始まり）の正解位置 = `(N + k) mod 4`、N = 対象 issue/PR 番号、
   0=a / 1=b / 2=c / 3=d。5 問なら `(N+1)%4` → `(N+2)%4` → … → `(N+5)%4` の順。
   選択肢を並べるとき**この位置に正解を置いてから**残りを埋める。生成者の裁量で
   位置を変えてはならない。
   **worked example（N=258）**: k=1→(258+1)%4=3→**d** / k=2→(258+2)%4=0→**a** /
   k=3→(258+3)%4=1→**b** / k=4→(258+4)%4=2→**c** / k=5→(258+5)%4=3→**d**
   → 5 問の正解位置列 `d a b c d`。生成時はこの計算を手元で行い、配置が一致しているか確認する。
   **正解の記法（正本フォーマット）**: `<details><summary>答えと解説</summary>` 展開後の
   最初の行を `**a**` / `**b**` / `**c**` / `**d**`（**小文字のみ**）で始める。
   大文字 A–D 形式は廃止——新規教材では使わない（照合コマンドが `[a-d]` に限定するため、
   大文字が混入すると誤 PASS になる）。
...
### Quiz 正解位置 照合コマンド（生成・検収の単一入口）

生成 agent の自己点検と監査役の検収が**同一コマンドを使う**（入口を 1 つに固定）。
照合コマンドを変えたくなったら ESCALATE する（自己判断で書き換えない）。

```bash
# N=対象 issue/PR 番号  FILE=explains/ 以下の教材 MD ファイルパス
N=<番号>; FILE=<ファイルパス>
k=0; errs=0
while IFS= read -r ans; do
  k=$((k+1)); exp=$(printf 'abcd' | cut -c$(( (N+k)%4+1 )))
  [ "$ans" = "$exp" ] \
    && echo "Q${k} OK (${ans})" \
    || { echo "Q${k} NG got=${ans} exp=${exp}  [N=${N} (N+${k})%4=$(( (N+k)%4 ))]"; errs=$((errs+1)); }
done < <(grep -h -A3 '<summary>答えと解説</summary>' "$FILE" \
          | grep -oE '^\*\*[a-d]' | sed 's/^\*\*//')
[ "$errs" -eq 0 ] && echo "PASS" || echo "FAIL ($errs errors)"
```

不一致（NG）が 1 件でもあれば不合格——再生成する。

- **生成 agent（publish 前・必須）**: 禁則語 grep（昨日/門番/運命/勘所 等）・4 節の存在・
  `<details>` の開閉対応・mermaid 構文の自己点検（skill の構文規律に対する機械照合）・
  **Quiz 正解位置の照合**: 上記照合コマンドを `N=対象番号 FILE=<作成した MD ファイル>` で
  実行し PASS を確認してから publish する（単一入口）。
- **監査役（publish 後・ブラウザ系ツールを持つ環境で）**: 実描画確認（mermaid iframe が
  ...
  あわせて **Quiz 正解位置の検査**: 上記照合コマンドを `N=対象番号 FILE=<教材 MD>` で
  実行し PASS を確認する（単一入口）。不一致があれば不合格（再生成）。
