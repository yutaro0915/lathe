...
- **`[INVLD]`（invalid）は fail と混ぜない**: run.mjs の 5 値報告（ADR 0022）で invalid＝判定不能（帰属=harness が併記される）。これはコードの失敗ではないので RED 一覧に足さず、`INVALID: <check> — <帰属と故障内容>` として別枠で報告する（切り分けは triage / outer の仕事）。
